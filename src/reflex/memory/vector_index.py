...
stores (tens of thousands of vectors). For larger deployments install ``reflex-memory[faiss]``
...
class FaissVectorIndex(VectorIndex):
    """FAISS-backed index (optional, ``reflex-memory[faiss]``) for large stores."""
    ...
    def __init__(self, dim: int, metric: Metric = "cosine") -> None:
        try:
            ...
            raise BackendError(
                'faiss is not installed. Install with `pip install "reflex-memory[faiss]"`.'
            ) from exc
        self._faiss = faiss
        self.dim = dim
        self.metric = metric
